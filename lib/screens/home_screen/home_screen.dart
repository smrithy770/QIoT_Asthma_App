...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _topSection() async {
    try {
      ...
      if (status == 200) {
        setState(() {
          _baseLineScore =
              jsonResponse['payload'][0]['user']['baseLineScore'].toString();
          _steroidDosage =
              jsonResponse['payload'][0]['user']['steroidDosage'].toString();
          _salbutomalDosage =
              jsonResponse['payload'][0]['user']['salbutomalDosage'].toString();
        });
      }
  ...
  Future<void> _bottomSection() async {
    try {
      final response =
          await UserApi().getAllAsthmaMessages(userModel!.accessToken);
      final jsonResponse = response;
      final status = jsonResponse['status'];
      if (status == 200) {
        int randomNumber = Random()
            .nextInt(jsonResponse['payload'][0]['asthmaMessages'].length);
        setState(() {
          _asthmamessages = jsonResponse['payload'][0]['asthmaMessages']
